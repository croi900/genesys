...
class CallHolder:
    def __init__(self, clazz, runid, mu, std, num_param=1, num_runs=1):
    ...
    def _combute_bbn_model(self, clazz, runid, mu, std, num_param, num_runs):
        def comp():
            ...
            if num_param == 1:
                ...
                param = np.random.normal(mu, std, (2,))
            ...
            if res is not None:
                print(param)
                DB.add_bbn(clazz.to_string(), runid, res, theta=str(param))

        Parallel(n_jobs=-1, verbose=55)(delayed(comp)() for _ in range(num_runs))

    def call(self):
        self._combute_bbn_model(
            self.model_class,
            self.runid,
            self.mu,
            self.std,
...
calls = [
    ...
    CallHolder(
        M1VPHI24,
        0,
        (-0.005509549623843782, 2.1985050873586847),
        (0.0023895031821490665, 0.9584276393664122),
        2,
        10000,
    ),
    CallHolder(
        M1VEXP,
        0,
        (2.824530644374782, 2.515918196865204),
        (0.05339503015951905, 1.1824084883834685),
        2,
        10000,
    ),
    ...
    CallHolder(
        M2VPHI24,
        0,
        (-0.000407619815153719, 0.3226697100165289),
        (0.00019527345867162578, 0.12905472738339577),
        2,
        10000,
    ),
    CallHolder(
        M2VEXP,
        0,
        (2.8949689209401708, 1.5628342940170938),
        (0.051284796651672855, 0.6941707304567979),
        2,
        10000,
    ),
    ...
    CallHolder(
        M3VPHI24,
        0,
        (-3.0237504571428573e-05, 0.5838196933333333),
        (2.0157890494672115e-05, 0.20160200591133598),
        2,
        10000,
    ),
    CallHolder(
        M3VEXP,
        0,
        (2.970619408993576, 1.693317008137045),
        (0.013936769624759645, 0.6312819082150594),
        2,
        10000,
    ),
]
