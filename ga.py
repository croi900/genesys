...
class GeneticAlgorithm:
    def __init__(self, model_class, npar, varlo, varhi,
                 maxit, popsize, threshold, mutrate, selection,
                 stagnation_limit,
                 freezeout_lower=3.068, freezeout_threshold=0.00135,
        ...
        self.reasons_path = f"logs/reasons" \
    ...
    def works_with_potential(self,candidate, model_class):
        try:
            ...
            thetas = list(
                np.random.uniform(low=-3, high=3, size=1)) + list(
                candidate)
            ...
            thetas = list(
                np.random.uniform(low=-3, high=3, size=2)) + list(
                candidate)
    ...
    def try_works_with_potential(candidate, model_class):
        ...
        results = Parallel(n_jobs=4)(
            delayed(works_with_potential)(candidate, model_class) for _ in
            range(100)
        )
    ...
    def validate_candidate(self, candidate):
        ...
        try:
            ...
            if not model_instance.valid:
                reasons = model_instance.reasons if hasattr(model_instance,
                                                            'reasons') else [
                    "Model invalid"]
            ...
            if not try_works_with_potential(candidate, self.model_class):
                return False, [
                    "failed in try_works_with_potential"], model_instance
    ...
    def param_fitness(self, candidate):
        ...
        try:
            ...
            ww_penalty = np.sum(
                np.abs(ww_vals[~((ww_vals > -1) & (ww_vals < 1))]))
            ...
            rr_penalty = np.abs(
                np.sum(model_instance.rr[model_instance.rr < 0]))
            
            phi_penalty = np.abs(
                np.sum(model_instance.phi[model_instance.phi < 0]))
            
            avg_rho = np.mean(model_instance.rho_w)
            rho_mag_penalty = 1 / avg_rho if avg_rho != 0 else 1e6

            total_penalty = ww_penalty + freezeout_penalty + rr_penalty + phi_penalty + rho_mag_penalty
    ...
    def filter_valid_population(self, population):
        ...
        results = Parallel(n_jobs=8)(
            delayed(self.validate_candidate)(candidate) for candidate in
            population
        )
        valid_population = []
        ...
        if not valid_population:
            print(
                "No valid candidates found! Generating a new random population.")
            return np.random.uniform(self.varlo, self.varhi,
                                     size=(self.popsize, self.npar))
    ...
    def generate_valid_population(self):
        ...
        while True:
            ...
            if len(valid_pop_and_fits) >= 2:
                ...
                print(
                    "Not enough valid candidates. Regenerating population...\n")
    ...
    def mutate(self, population_slice):
        ...
        for i in range(len(population_slice)):
            if random.random() < self.mutrate:
                ...
                population_slice[i, col] = np.random.uniform(self.varlo,
                                                             self.varhi)
        ...
        while generation < self.maxit:
            try:
                ...
                new_fits = Parallel(n_jobs=-1)(
                    delayed(self.param_fitness)(candidate) for candidate in
                    new_offspring
                )
                valid_offspring = [(child, f) for child, f in
                                   zip(new_offspring, new_fits) if
                                   f is not None]
