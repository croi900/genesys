...
class M2VPHI24(M2V0):
    ...
    def _compute_beta(self):
        self.beta = (np.log(3 - self.alpha ** 2 / 2 * (
                    self.lam + 3 * self.h0 * self.psi01 + self.psi01 ** 2))
                     - np.log(
                    self.phi01 ** 2 / 2 + self.x * self.phi0 ** 2 + self.z * self.phi0 ** 4)) / self.phi0
    ...
    def _system(self, t, Y):
        ...
        d_phi_2 = -3 * h * phi_prime - 2 * self.x * phi - self.z * 4 * phi ** 3 - self.beta * phi_prime ** 2 - self.beta * self._v(
            phi)
