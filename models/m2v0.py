...
class M2V0(PotentialModel):
    ...
    @staticmethod
    def get_initials():
        return [-0.03686985,-0.01737777,0.04948915,-0.04746112,0.0238886,0.03729411]
    ...
    def _system(self, t, Y):
        ...
        d_phi_2 = -3 * h * phi_prime - self._v(
            phi) - self.beta * phi_prime ** 2 - self.beta * self._v(phi)
        ...
        d_h = 0.5 * (
                -3 * h ** 2
                - rr
                - np.exp(self.beta * phi) * (0.5 * phi_prime ** 2 - self._v(
            phi))
