...
class M3V0(PotentialModel):
    ...
    @staticmethod
    def get_initials():
        return [-0.00938917,-0.00093121,-0.08795092,0.0155224,0.02328965,-0.04948566]

    def _compute_beta(self):
        self.beta = (np.log(
        3 - self.alpha ** 2 / 6 * (self.lam + 3 * self.h0 * self.psi01 + self.psi01 ** 2)) - np.log(
        self.phi01 ** 2 / 2 + 0)) / self.phi0
    ...
    def _system(self, t, Y):
        ...
        d_phi_2 = -3 * h * phi_prime - self.beta * phi_prime ** 2 - self.beta * self._v(
            phi)

        d_h = 0.5 * (-3 * h ** 2 - rr - np.exp(self.beta * phi) * (
                    0.5 * phi_prime ** 2 - self._v(phi))
        ...
        psi_source = (self.beta / (
                    3 * self.alpha ** 2)) * phi_prime ** 3 * np.exp(
            self.beta * phi) - h * psi_prime ** 2
