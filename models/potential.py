...
class PotentialModel(Model):
    def __init__(self, p_phi0, p_phi01, p_psi0, p_psi01, p_alpha, p_lam,
                 lambda_dpsi2=False):
    ...
    def _compute_beta(self):
        self.beta = (np.log(3.0 - self.alpha ** 2 / 2.0 * self.lam)
                     - np.log(self.phi01 ** 2 / 2.0)) / self.phi0
    ...
    def _solve(self):
        try:
            ...
            solution = solve_ivp(lambda t, Y: self._system(t, Y), t_span,
                                  self.initial_guess,
                                 t_eval=self.theta,
                                 method=self.solver,
                                 rtol=self.rtol,
            ...
            if solution == None or np.isnan(solution.y).any() or np.isinf(solution.y).any():
    ...
    def _compute_functions_time(self):
        rho_phi = np.exp(self.beta * self.phi) * (0.5 * self.dphi ** 2 +
                                                  self._v(self.phi))
        p_phi = np.exp(self.beta * self.phi) * (0.5 * self.dphi ** 2 -
                                                self._v(self.phi))
        ...
        self.p_w = p_phi - (self.alpha ** 2 / 2) * (dpsi2_f + 3 * self.h *
                                                    self.dpsi)
    ...
    def _compute_time_map(self):
        ...
        self.rho_w_extended = np.concatenate(
            (self.rho_w, np.ones_like(
                self.theta_extended[len(self.rho_w):len(self.theta_extended)]) *
             self.rho_w[-1]))

        self.p_w_extended = np.concatenate(
            (self.p_w, np.ones_like(self.theta_extended[len(self.p_w):len(
                self.theta_extended)]) * self.p_w[-1]))

        self.drho_dt_w_extended = np.concatenate(
            (self.drho_dt_w, np.ones_like(self.theta_extended[len(self.p_w):len(
                self.theta_extended)]) * self.drho_dt_w[-1]))

        h_int_f = cumulative_trapezoid(self.h_extended, self.theta_extended,
                                       initial=0.007386520369079301)
        T0 = 10
        T_vals = T0 * np.exp(-h_int_f)

        self.tofT = interp1d(T_vals[::-1], self.theta_extended[::-1],
                             kind='linear',
                             bounds_error=False, fill_value="extrapolate")

        self.Toft = interp1d(self.theta_extended, T_vals, kind='linear',
                             bounds_error=False,
    ...
    def _interpolate(self):
        ...
        self.time.drho_dt_w = self._lerp(self.theta_extended,
                                       self.drho_dt_w_extended)

        self.temp.rho_w = lambda t: self.time.rho_w(self.tofT(t))  * MeV**4 * MeV4_to_gcmm3
        self.temp.p_w = lambda t: self.time.p_w(self.tofT(t))  * MeV**4 * MeV4_to_gcmm3
        self.temp.drho_dt_w = lambda t: self.time.drho_dt_w(self.tofT(t))  * MeV**4 * MeV4_to_gcmm3
    ...
    def compute_abundances(self):
        if self.temp.rho_w is None or self.temp.p_w is None or self.temp.drho_dt_w is None:
        ...
        try:
            prym = PRyMmain.PRyMclass(self.temp.rho_w, self.temp.p_w,
                                  self.temp.drho_dt_w)
